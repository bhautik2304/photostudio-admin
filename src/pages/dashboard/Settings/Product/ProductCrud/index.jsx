...
import { Container, Stack, Button, Paper, Autocomplete, Grid, Typography, Box, Accordion, AccordionActions, AccordionSummary, AccordionDetails, DialogContentText, TextField, FormControl, FormLabel, Dialog, Card, CardHeader, CardContent, InputAdornment, IconButton, DialogTitle, DialogContent, DialogActions, colors } from '@mui/material';
    ...
    <>
      ...
      <Container maxWidth={themeStretch ? false : 'lg'}>
          ...
          <Grid container >
            <Grid sm={6} xs={12} lg={6}>
              <Stack flex='colum' spacing={2} padding={5} >
                ...
                {
                  (countryzone && productData) && countryzone.map((data, key) => {
                    console.log(data);
                    return (
                      <FormControl>
                        <FormLabel>
        ...
        <Card sx={{ marginY: 5, padding: 2 }} >
          ...
          {productData ? productData?.orientation.map((data, key) => <Orientation key={key} data={data} setAddOrientationModelShow={setAddOrientationModelShow} />) : null}
        </Card>
      </Container>

      <AddOrientationModel show={addOrientationModelShow} productId={productData?.id} onClose={() => setAddOrientationModelShow(false)} />
    </>
  )
}
...
const AlbumCopyPriceField = ({ sign, productId, zoneId, pricrData }) => {
  ...
  const albumcopyPrice = pricrData.find(data => (data.product_id === productId) && (data.countryzone_id === zoneId))
  ...
  const createPrice = () => {
    setLoad(true)
    axios.post(`${apiRoutes.productReq}productalbumcopyprice/`, data).then((e) => {
  ...
  const updatePrice = () => {
    setLoad(true)
    axios.put(`${apiRoutes.productReq}productalbumcopyprice/${albumcopyPrice?.id}`, data).then((e) => {
      setLoad(false)
      dispatch(fetchproduct())
    }).catch((e) => {
      setLoad(false)
  ...
  return (
    <>
      <Accordion sx={{ margin: 2, backgroundColor: colors.grey }} elevation={3} >
        ...
        <AccordionDetails>
          <CardContent>
            {data?.size.map((datas, key) => <Size data={datas} />)}

          </CardContent>
        </AccordionDetails>
      </Accordion>
      <AddSizeModel show={addSizeModelShow} onClose={() => setAddSizeModelShow(false)} productOrientationId={data.id} />
    </>
  )
}
        ...
        <AccordionDetails>
          ...
          <Accordion sx={{ backgroundColor: colors.grey, }} elevation={3} >
            ...
            >
              ...
              <CardHeader title={
                <Stack direction='row' alignItems='center' spacing={3} justifyContent='space-between' sx={{ width: '100%', padding: 0 }}>
            ...
            <AccordionDetails>
              <CardContent>
                {data?.sheet.map((datas, key) => <ProductSheet data={datas} />)}
              </CardContent>
            </AccordionDetails>
          </Accordion>
          ...
          <Accordion sx={{ marginY: 5, backgroundColor: colors.grey, }} elevation={3} >
            ...
            <AccordionDetails>
              <CardContent>
                {data?.papers.map((datas, key) => <ProductPapers data={datas} />)}
              </CardContent>
            </AccordionDetails>
          </Accordion>
          ...
          <CardContent>
            {data?.cover.map((datas, key) => <ProductCover data={datas} />)}
          </CardContent>
          ...
          <CardContent>
            {data?.boxsleeve.map((datas, key) => <ProductBoxsleeve data={datas} />)}
          </CardContent>
        </AccordionDetails>
      </Accordion>

      <AddsheetModel sizeId={data.id} show={addsheetShow} onClose={() => setAddsheetShow(false)} />
      <AddPapersModel sizeId={data.id} show={addPapersShow} onClose={() => setAddPapersShow(false)} />
      <AddCoverModel sizeId={data.id} show={addCoverShow} onClose={() => setAddCoverShow(false)} />
      <AddBoxsleeveModel sizeId={data.id} show={addBoxsleeveShow} onClose={() => setAddBoxsleeveShow(false)} />
    </>
  )
}
      ...
      <Stack spacing={3} sx={{ marginTop: 3 }} direction='row' >
        ...
        {countryzone.map((datas, key) =>
          <PriceTextField
            ...
            zoneId={datas.id}
            parentId={data.id}
            price={data.sheetprice.find((val) => Number(val.countryzone_id) === Number(datas.id))?.price || 0}
            productResourcePriceId={data.sheetprice.find((val) => Number(val.countryzone_id) === Number(datas.id))?.id || null}
...
const PriceTextField = ({ price, label, sign, zoneId, productResourcePriceId, parentId, url }) => {
  ...
  const createPrice = () => {
    setLoad(true)
    axios.post(`${apiRoutes.productReq}${url}/`, data).then((e) => {
      dispatch(fetchproduct())
      setLoad(false)
    }).catch((e) => {
      setLoad(false)
  ...
  const updatePrice = () => {
    setLoad(true)
    axios.put(`${apiRoutes.productReq}${url}/${productResourcePriceId}`, data).then((e) => {
      setLoad(false)
      dispatch(fetchproduct())
    }).catch((e) => {
      setLoad(false)
  ...
  return (
    <Grid container >
      <Stack spacing={3} sx={{ marginTop: 3 }} direction={{ lg: 'row', sm: 'row', xs: 'column' }} >
        ...
        <Grid item sm='3' xs='12' lg='3' >
          <TextField value={data.paper.value} InputProps={{
            endAdornment:
      ...
      <Stack spacing={3} sx={{ marginTop: 3 }} direction='row' >
        ...
        {countryzone.map((datas, key) =>
          <PriceTextField
            ...
            zoneId={datas.id}
            parentId={data.id}
            price={data.coverprice.find((val) => Number(val.countryzone_id) === Number(datas.id))?.price || 0}
            productResourcePriceId={data.coverprice.find((val) => Number(val.countryzone_id) === Number(datas.id))?.id || null}
            label={`Price in ${datas.zonename}`}
            sign={datas.currency_sign}
      ...
      <Stack spacing={3} sx={{ marginTop: 3 }} direction='row' >
        ...
        {countryzone.map((datas, key) =>
          <PriceTextField
            ...
            zoneId={datas.id}
            parentId={data.id}
            price={data.boxsleeveprice.find((val) => Number(val.countryzone_id) === Number(datas.id))?.price || 0}
            productResourcePriceId={data.boxsleeveprice.find((val) => Number(val.countryzone_id) === Number(datas.id))?.id || null}
            label={`Price in ${datas.zonename}`}
            sign={datas.currency_sign}
...
const AddOrientationModel = ({ show, onClose, productId }) => {
  ...
  const dataArry = orientation && orientation?.map((data) => ({ id: data.id, label: data.name })) || []
