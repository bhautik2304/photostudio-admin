...
// @mui
import { Box, Grid, Card, Stack, Typography, FormControlLabel, Switch, TextField, Select, MenuItem, Button, FormControl, InputLabel } from '@mui/material';
...
export default function AccountGeneral() {
  ...
  const updateZone = () => {
    const updateSTate = toast("Zone Updating")
              ...
              <FormControlLabel
                ...
                control={
                  <Switch
                    ...
                    onChange={(event) => {
                      axios.post(`${apiRoutes.costomer}status/${id}`, { status: event.target.checked ? 1 : 0 }).then(res => {
                        enqueueSnackbar('Profile Updated', { variant: 'success' });
                        setIsSubmitting(false)
                        disapatch(fetchCustomer())
                      }
              ...
              <FormControlLabel
                ...
                control={
                  <Switch
                    ...
                    onChange={(event) => {
                      axios.post(`${apiRoutes.costomer}approve/${id}`).then(res => {
                        enqueueSnackbar('Profile Updated', { variant: 'success' });
                        setIsSubmitting(false)
                        disapatch(fetchCustomer())
                      }
